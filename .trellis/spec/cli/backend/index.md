...
| [Trellis Core SDK](./trellis-core-sdk.md)               | `@ecochain/trellis-core` / CLI package boundary, public exports, build and versioning contracts                                                                                                   | Done   |
| [Governance Gate](./governance-gate.md)                 | Classified task create/start gates, Task Basis contracts, evaluator protocol, Dazz presentation boundaries, and tests                                                                             | Done   |
...
- Editing `packages/core/**`, moving reusable CLI logic into core, or changing CLI imports from `@ecochain/trellis-core` → [trellis-core-sdk.md](./trellis-core-sdk.md)
- Editing task classification, `intent.md` Task Basis parsing, governance config, or task create/start gates → [governance-gate.md](./governance-gate.md)
...
4. Choose verification according to behavior and risk:
   - TDD is optional; prefer it when reproducible behavior can be stated before implementation.
   - Reproducible regressions should receive a durable automated prevention check when practical.
   - Changed init/update behavior normally needs focused integration evidence.
   - If a test category is not applicable, record why and provide proportionate alternative evidence.
