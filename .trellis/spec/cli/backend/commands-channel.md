...
`@ecochain/trellis-core/channel` owns reusable channel domain behavior:
...
metadataFromCreateEvent(ev?): ChannelMetadata
  ...
  // @ecochain/trellis-core/channel and do not call from CLI renderers.
...
**Worker lifecycle / inbox / delivery contracts** (owned by `@ecochain/trellis-core`):
...
- Trigger: `@ecochain/trellis-core` mutation APIs need replay safety for
