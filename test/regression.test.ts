...
describe("regression: start and brainstorm templates preserve index-first contract (2026-04)", () => {
  it("Claude and Codex start templates keep index-first context while allowing scoped Dazz guidance", () => {
    ...
    for (const content of [claudeStart as string, codexStart as string]) {
      ...
      expect(content).toContain(
        "If the injected context reports blockers, stop and explain the prerequisite in plain language.",
      );
  ...
  it("iFlow and Codex brainstorm templates keep injected-context flow while allowing scoped Dazz guidance", () => {
    ...
    for (const content of [iflowBrainstorm as string, codexBrainstorm as string]) {
      ...
      expect(content).toMatch(
        /If `(?:\/trellis:start|\$start)` surfaced blockers in the injected session gate summary or referenced indexes:/,
      );
      expect(content).toContain("Do not hardcode package-specific gates here.");
      expect(content).toContain(
