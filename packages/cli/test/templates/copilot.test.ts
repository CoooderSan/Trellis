...
import { collectCopilotTemplates } from "../../src/configurators/copilot.js";
...
describe("copilot getAllPrompts", () => {
  ...
  it("generated check skill uses the common quality contract", () => {
    const check = collectCopilotTemplates().get(
      ".github/skills/trellis-check/SKILL.md",
    );
    expect(check).toBeDefined();
    expect(check).toContain("TDD is optional and risk-driven");
    expect(check).toContain("alternative verification evidence");
    expect(check).toContain("`ITERATION`-profile verification");
    expect(check).toContain("`MR_CANDIDATE` claim additionally requires");
    expect(check).not.toContain("New function → unit test added?");
    expect(check).not.toContain("Bug fix → regression test added?");
  });
});
