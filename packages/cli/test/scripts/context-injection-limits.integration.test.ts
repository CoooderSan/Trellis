...
function runTaskValidateRole(
  tmp: string,
  taskDir: string,
  role: "implement" | "check" = "implement",
): { status: number | null; stdout: string; stderr: string } {
  const r = spawnSync(
    "python3",
    [
...
`,
  ...
  describe("task.py validate: archived task paths (#518)", () => {
    ...
    it.skipIf(process.platform === "win32")(
      "rejects an archived self-directory whose Markdown file symlink escapes the archive",
      () => {
        const taskDir = makeArchivedTask(tmp, taskName);
        const researchDir = path.join(taskDir, "research");
        const outsideFile = path.join(tmp, "docs", "outside.md");
        fs.mkdirSync(researchDir, { recursive: true });
        fs.mkdirSync(path.dirname(outsideFile), { recursive: true });
        fs.writeFileSync(path.join(researchDir, "safe.md"), "safe\n", "utf-8");
        fs.writeFileSync(outsideFile, "outside\n", "utf-8");
        fs.symlinkSync(outsideFile, path.join(researchDir, "escape.md"));
        fs.writeFileSync(
          path.join(taskDir, "implement.jsonl"),
          JSON.stringify({
            file: `.trellis/tasks/${taskName}/research/`,
        ...
        const { status, stderr } = runTaskValidateRole(tmp, taskDir);

        expect(status).toBe(1);
        expect(stderr).toContain("referenced path escapes the task archive");
        expect(stderr).toContain("manifest has no valid readable 'file' entry");
      },
    );

    it.skipIf(process.platform === "win32")(
      "rejects an archived self-directory whose Markdown-named directory symlink escapes the archive",
      () => {
        const taskDir = makeArchivedTask(tmp, taskName);
        const researchDir = path.join(taskDir, "research");
        const outsideDir = path.join(tmp, "docs", "outside-directory");
        fs.mkdirSync(researchDir, { recursive: true });
        fs.mkdirSync(outsideDir, { recursive: true });
        fs.writeFileSync(path.join(researchDir, "safe.md"), "safe\n", "utf-8");
        fs.symlinkSync(outsideDir, path.join(researchDir, "escape.md"));
        fs.writeFileSync(
          path.join(taskDir, "implement.jsonl"),
          JSON.stringify({
            file: `.trellis/tasks/${taskName}/research/`,
    ...
    it("accepts historical backslash file and directory self-references after archival", () => {
      const taskDir = makeArchivedTask(tmp, taskName);
      const researchDir = path.join(taskDir, "research");
      fs.mkdirSync(researchDir, { recursive: true });
      fs.writeFileSync(
        path.join(researchDir, "evidence.md"),
        "archived evidence\n",
        "utf-8",
      );
      fs.writeFileSync(
        ...
        [
          JSON.stringify({
            file: `.trellis\\tasks\\${taskName}\\research\\evidence.md`,
            reason: "historical file",
          }),
          JSON.stringify({
            file: `.trellis\\tasks\\${taskName}\\research\\`,
            type: "directory",
      ...
      const { status, stdout, stderr } = runTaskValidateRole(tmp, taskDir);

      expect(status).toBe(0);
      expect(stdout).toContain("Role context ready: implement.jsonl");
      expect(stderr).toBe("");
    });
