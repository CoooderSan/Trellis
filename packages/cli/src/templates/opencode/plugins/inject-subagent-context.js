...
class RoleContextNotReadyError extends Error {}

function readableEntryError(ctx, entryPath, entryType, taskDirFull) {
  try {
    const entryStat = statSync(entryPath);
    if ((entryStat.mode & 0o444) === 0) {
      return "cannot be read: no read permission";
    }
    if (entryType === "directory") {
      if (!entryStat.isDirectory()) return "is not a directory";
      ctx.resolveContextDirectoryFiles(entryPath, taskDirFull);
      return null;
    }
    if (!entryStat.isFile()) return "is not a file";
    readFileSync(entryPath).subarray(0, 1);
    return null;
  } catch (error) {
    const detail = error instanceof Error ? error.message : String(error);
    return `cannot be read: ${detail}`;
  }
}

function roleContextReadinessErrors(ctx, taskDir, role) {
  const manifestName = `${role}.jsonl`;
  const taskDirFull = ctx.resolveTaskDir(taskDir);
  const manifestPath = taskDirFull && join(taskDirFull, manifestName);
  if (!manifestPath) return [`${manifestName}: task directory is unresolved`];

  let manifest;
  try {
    const manifestStat = statSync(manifestPath);
    if (!manifestStat.isFile()) {
      return [`${manifestName}: manifest is not a file`];
    }
    if ((manifestStat.mode & 0o444) === 0) {
      return [`${manifestName}: manifest cannot be read: no read permission`];
    }
    const data = readFileSync(manifestPath);
    if (!isUtf8(data)) {
      return [`${manifestName}: manifest is not valid UTF-8`];
    }
    manifest = data.toString("utf-8");
  } catch (error) {
    if (!existsSync(manifestPath)) {
      return [`${manifestName}: manifest is missing`];
    }
    const detail = error instanceof Error ? error.message : String(error);
    return [`${manifestName}: manifest cannot be read: ${detail}`];
  }
  ...
  for (const [index, line] of manifest.split(/\r?\n/).entries()) {
    ...
    try {
      ...
      const detail = error instanceof Error ? error.message : String(error);
      errors.push(`${manifestName}:${index + 1}: invalid JSON: ${detail}`);
      continue;
    }
    if (!entry || typeof entry !== "object" || Array.isArray(entry)) {
      errors.push(`${manifestName}:${index + 1}: entry must be a JSON object`);
      continue;
    }

    const rawFile = entry.file;
    if (rawFile == null && Object.hasOwn(entry, "_example")) continue;
    if (typeof rawFile !== "string" || !rawFile.trim()) {
      errors.push(
        `${manifestName}:${index + 1}: entry requires a non-empty string field 'file'`,
      );
      continue;
    }
    ...
    if (entryType !== "file" && entryType !== "directory") {
      errors.push(
        `${manifestName}:${index + 1}: type must be 'file' or 'directory'`,
      );
      continue;
    }

    const file = rawFile.trim();
    const entryPath = ctx.resolveContextEntryPath(file, taskDirFull);
    if (!entryPath) {
      errors.push(
        `${manifestName}:${index + 1}: referenced path escapes the task archive: ${file}`,
      );
      continue;
    }
    const unreadable = readableEntryError(
      ctx,
      entryPath,
      entryType,
      taskDirFull,
    ...
    if (unreadable) {
      errors.push(
        `${manifestName}:${index + 1}: referenced ${entryType} ${unreadable}: ${file}`,
      );
      continue;
    }
    validEntries += 1;
  ...
  if (nonEmptyRows === 0) errors.push(`${manifestName}: manifest is empty`);
  if (validEntries === 0) {
    errors.push(
      `${manifestName}: manifest has no valid readable 'file' entry (seed rows do not count)`,
    );
  }
  return errors;
}

function assertRoleContextReady(ctx, taskDir, role) {
  const errors = roleContextReadinessErrors(ctx, taskDir, role);
  if (errors.length === 0) return;

  const manifestName = `${role}.jsonl`;
  throw new RoleContextNotReadyError(
    [
      `Trellis blocked trellis-${role} dispatch because ${manifestName} is not ready:`,
      ...errors.map((error) => `- ${error}`),
      `Curate ${manifestName} with at least one readable {"file":"...","reason":"..."} entry, then retry the dispatch.`,
    ].join("\n"),
  );
}
    ...
    "tool.execute.before": async (input, output) => {
      try {
        ...
        if (AGENTS_REQUIRE_TASK.includes(subagentType)) {
          ...
          if (!taskDir) {
            throw new RoleContextNotReadyError(
              `Trellis blocked trellis-${subagentType} dispatch because no active task could be resolved. Add an "Active task: .trellis/tasks/<task>" line to the dispatch prompt or start the task, curate ${subagentType}.jsonl, then retry.`,
            );
          }
          const taskDirFull = ctx.resolveTaskDir(taskDir);
          if (!taskDirFull || !existsSync(taskDirFull)) {
            throw new RoleContextNotReadyError(
              `Trellis blocked trellis-${subagentType} dispatch because the active task directory does not exist: ${taskDir}. Fix the active task reference, curate ${subagentType}.jsonl, then retry.`,
            );
          }
          assertRoleContextReady(ctx, taskDir, subagentType);
        }
        ...
        if (error instanceof RoleContextNotReadyError) throw error;
      }
    },
  };
};
