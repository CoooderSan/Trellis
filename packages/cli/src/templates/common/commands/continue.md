...
- `status=planning` + missing `intent.md` or `prd.md` → **1.1** (load `trellis-brainstorm` and complete the Intent/PRD artifacts)
- `status=planning` + `intent.md` and `prd.md` only → decide whether the task is lightweight or complex. Lightweight can move to **1.4** review; complex returns to **1.1** to add `design.md` + `implement.md`.
...
2. `[once]` steps are already done if the required output exists. `intent.md` + `prd.md` can be enough only for lightweight tasks; complex tasks also need `design.md` and `implement.md`.
