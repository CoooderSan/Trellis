...
/**
 * Write a task skeleton (task.json + intent.md + prd.md).
 *
...
function writeTaskSkeleton(
  ...
  try {
    ...
    fs.writeFileSync(
      path.join(taskDir, FILE_NAMES.INTENT),
      intentContent,
      "utf-8",
    );
...
function getBootstrapIntentContent(
  projectType: ProjectType,
  packages?: DetectedPackage[],
): string {
  const packageScope = packages?.length
    ? packages.map((pkg) => pkg.name).join(", ")
    : projectType;

  return `# Intent: Bootstrap Guidelines

## Intent

Initialize Trellis project guidelines so future AI-assisted development follows the repository's real conventions.

## Source

- Trellis init created this first-time setup task.
- Project scope: ${packageScope}

## Scope

- In scope: inspect the codebase, fill .trellis/spec/, and record concrete examples.
- Out of scope: product feature implementation and unrelated refactors.

## Acceptance Criteria

- [ ] Guidelines are filled with real repository conventions.
- [ ] Future tasks can load package/layer specs through Trellis context.

## Risk

- Level: Low
- Approval: Trellis bootstrap task created during init.
`;
}
...
- Each task has \`implement.jsonl\` / \`check.jsonl\` manifests listing which
  spec files to load.
- The platform hook auto-injects those spec files + the task's \`intent.md\` / \`prd.md\`
...
function createBootstrapTask(
  ...
  const intentContent = getBootstrapIntentContent(projectType, packages);
...
function getJoinerIntentContent(developer: string): string {
  return `# Intent: Join Trellis Project

## Intent

Onboard ${developer} to the existing Trellis workflow, project conventions, and current task state.

## Source

- Trellis init detected this developer joining an existing Trellis project.

## Scope

- In scope: explain workflow mechanics, available specs, active tasks, and how to start work safely.
- Out of scope: product feature implementation and repository refactors.

## Acceptance Criteria

- [ ] Developer understands the local Trellis workflow and where project specs live.
- [ ] Developer can identify the next task or ask for a specific walkthrough.

## Risk

- Level: Low
- Approval: Trellis joiner onboarding task created during init.
`;
}
...
- **Three phases**: Plan (brainstorm → \`intent.md\` + \`prd.md\`) → Execute (code + check) →
...
- **\`/trellis:continue\`** loads the Phase Index, reads \`intent.md\` + \`prd.md\` + recent
...
- **\`trellis-implement\` sub-agent** is spawned when code needs to be written.
  ...
  spec files + \`intent.md\` / \`prd.md\` into the sub-agent's prompt so it codes per project
  conventions.
...
function createJoinerOnboardingTask(
  ...
  const intentContent = getJoinerIntentContent(developer);
  const prdContent = getJoinerPrdContent(developer, pythonCmd);
  return writeTaskSkeleton(cwd, taskName, taskJson, intentContent, prdContent);
}
