...
const MAX_CONTEXT_PART_OFFSET = Object.values(CONTEXT_PART_KINDS).reduce(
  (maximum, definition) =>
    definition.offset > maximum ? definition.offset : maximum,
  0n,
);
...
export function insertSyntheticTextPart(parts, text, kind) {
  ...
  if (!CONTEXT_PART_KINDS[kind])
    throw new TypeError(`unknown context part kind: ${kind}`);

  const source = findIdentitySourcePart(parts);
  if (!source)
    throw new TypeError("no ordinary OpenCode part with a persisted identity");
  ...
  const insertionIndex = parts.findIndex(
    (existing) => typeof existing?.id !== "string" || id < existing.id,
  );
