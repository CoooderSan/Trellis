...
4. When the blocker is missing Intent or incomplete scope, you may still do read-only research to help the user form the Intent.
5. If the user refuses to write Intent themselves, offer to draft the smallest viable Intent from available sources, but do not describe the work as proceeding without Intent.
6. If existing docs, specs, notes, or code can answer the next question, inspect them before asking the user.
7. If team governance already names a preferred knowledge source such as synced docs, check that source before asking the user to restate it.
8. Do not finish, archive, retire, or otherwise mutate the current task state while prerequisites for the next task are still unresolved.
9. Do not implement, create a task directory, or seed a PRD while prerequisites are still unresolved.
10. Continue with task classification only after the injected rule entry allows progress.
11. When governance is enabled, `task.py create` and `task.py start` also enforce this gate at runtime; do not try to bypass that failure.
