...
export function parseRegistrySource(source: string): RegistrySource {
  ...
  let codeupWrapped = false;
  let codeupPrefixed = false;
  if (source.startsWith("codeup:https://")) {
    codeupWrapped = true;
    sourceKind = "https";
    const wrapped = source.slice("codeup:".length);
    const httpsMatch = wrapped.match(
      /^https?:\/\/([^/]+)\/(.+?)(?:\.git)?\/?$/,
    );
    if (httpsMatch) {
      host = httpsMatch[1];
  ...
  let normalized = normalizedInput ?? normalizeRegistrySource(source);
  if (!normalizedInput && source.startsWith("codeup:")) {
    codeupPrefixed = true;
    sourceKind = "prefixed";
    host = CODEUP_HOST;
    normalized = `gitlab:${source.slice("codeup:".length)}`;
  }
  ...
  let subdir = segments.slice(2).join("/");
  if (host && sourceKind === "https" && !source.includes("/-/tree/")) {
    repo = segments.join("/");
    subdir = "";
  }
  if (codeupWrapped) {
    repo = segments.slice(0, -1).join("/");
    subdir = segments.at(-1) ?? "";
  }
  ...
  if (host === CODEUP_HOST && codeupPrefixed) {
    rawBaseUrl = `https://${CODEUP_HOST}/${repo}/raw/${ref}/${subdir}`;
  }

  // Build giget source (use normalized format)
  const gigetSource =
    host === CODEUP_HOST
      ? `gitlab:${repo}${subdir ? `/${subdir}` : ""}${ref === "main" && codeupPrefixed ? "" : `#${ref}`}`
      : normalized;
