...
4. **All** submodule commits are pushed before the main repo pointer commit (currently `docs-site/` + `marketplace/`). Fetch the team-owned remote and verify that each pointer is an ancestor of its remote branch with `git merge-base --is-ancestor`. Do not use `git ls-remote origin $sha`: it matches ref names, not commit reachability, and falsely fails for a normal pointer behind the branch tip. A controlled executor's recursive checkout fails on the first unreachable pointer with `fatal: remote error: upload-pack: not our ref <SHA>`.
